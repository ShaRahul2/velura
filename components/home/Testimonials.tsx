      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {REVIEWS.map(({ id, quote, author, location, product, rating }) => (
            <div
              key={id}
              className="flex flex-col gap-5 bg-cream p-7 rounded-card shadow-card"
            >
              <p
              ...
              <div className="mt-auto pt-4 border-t border-lm flex items-center justify-between">
                <div>
                  <p className="font-sans text-[0.78rem] font-medium text-deep">{author}</p>
                  <p className="font-sans text-[0.68rem] text-mauve">{location}</p>
                </div>
