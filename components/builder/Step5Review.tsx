...
function ReviewRow({ label, value }: ReviewRowProps) {
  return (
    <div className="flex items-center justify-between gap-1.5 py-0.5 border-b border-lm">
      <span className="font-sans text-[0.5rem] tracking-label uppercase text-mauve">{label}</span>
      <span className="font-sans text-[0.64rem] text-deep capitalize truncate">{value ?? <span className="text-mauve opacity-60">—</span>}</span>
    </div>
  )
}
...
export function Step5Review() {
  ...
  return (
    <div>
      ...
      <p className="font-sans text-[0.6rem] text-mauve mb-1.5">Looks right? Add to bag.</p>
      ...
      >
        ...
        <div className="flex justify-between pt-1 mt-1 border-t border-lm">
          <span className="font-sans text-[0.62rem] tracking-label uppercase text-deep">Total</span>
      ...
      <div className="space-y-1.5">
        <button
          ...
          className="w-full h-8 font-sans text-[0.68rem] tracking-btn uppercase bg-deep text-blush hover:tracking-wide transition-all duration-200"
          style={{ borderRadius: 3 }}
        >
          Add to Bag · {formatPrice(price)}
        </button>
        <p className="font-sans text-[0.55rem] text-center text-mauve">
